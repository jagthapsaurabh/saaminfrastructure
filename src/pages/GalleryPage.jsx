...
import { ArrowLeft, ArrowRight, ArrowUpRight, X, ZoomIn, Minus, Plus, Filter } from "lucide-react";
...
function GalleryPage() {
  ...
  const nextImage = () => { if (selectedIndex === null) return; setSelectedIndex((prev) => (prev + 1) % filteredImages.length); setZoom(1); setPosition({ x: 0, y: 0 }); };
  const previousImage = () => { if (selectedIndex === null) return; setSelectedIndex((prev) => (prev - 1 + filteredImages.length) % filteredImages.length); setZoom(1); setPosition({ x: 0, y: 0 }); };
  ...
  const handleMouseDown = (e) => { if (zoom <= 1) return; e.preventDefault(); setIsDragging(true); setDragStart({ x: e.clientX - position.x, y: e.clientY - position.y }); };
  const handleMouseMove = (e) => { if (!isDragging || zoom <= 1) return; setPosition({ x: e.clientX - dragStart.x, y: e.clientY - dragStart.y }); };
  const handleMouseUp = () => setIsDragging(false);
  const handleTouchStart = (e) => { if (zoom <= 1) return; const t = e.touches[0]; setIsDragging(true); setDragStart({ x: t.clientX - position.x, y: t.clientY - position.y }); };
  const handleTouchMove = (e) => { if (!isDragging || zoom <= 1) return; const t = e.touches[0]; setPosition({ x: t.clientX - dragStart.x, y: t.clientY - dragStart.y }); };
  ...
  return (
    <main className="min-h-screen bg-[#F8F5ED] text-[#11130F]">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#1B2A4A] via-[#253D5E] to-[#1B2A4A]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-24 text-center sm:px-6 sm:py-32 lg:py-40">
          <div className="flex items-center justify-center gap-3">
            ...
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#C9A227]">Visual Portfolio</p>
            <span className="h-px w-8 bg-[#C9A227]" />
          </div>
          <h1 className="font-heading mt-6 text-3xl font-bold leading-tight text-white sm:text-4xl lg:text-5xl">
            Project<span className="text-[#C9A227]"> Gallery.</span>
          </h1>
          ...
          <div className="mx-auto mt-10 flex max-w-xs justify-center gap-4">
            {[
      ...
      <section className="sticky top-16 z-30 border-b border-[#E6E2D7] bg-white/95 shadow-sm backdrop-blur-md lg:top-[72px]">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="flex justify-center gap-2 overflow-x-auto py-4 scrollbar-none sm:flex-wrap sm:gap-3">
            ...
            {categories.map((cat) => (
              <button key={cat} type="button" onClick={() => { setActiveCategory(cat); setSelectedIndex(null); }}
                className={`shrink-0 rounded-full border px-5 py-2 text-xs font-bold uppercase tracking-wider transition-all duration-300 active:scale-95 sm:px-6 sm:text-sm ${
                  activeCategory === cat ? "border-[#C9A227] bg-[#C9A227] text-[#11130F] shadow-md" : "border-[#E6E2D7] bg-white text-[#6B7280] hover:border-[#C9A227] hover:text-[#A98216]"
                }`}
              >
                {cat}
              </button>
      ...
      {/* ════════════════════════════════════════════════════
          GALLERY GRID — Symmetrical 3-col
      ════════════════════════════════════════════════════ */}
      <section className="px-5 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <div className="flex items-center justify-center gap-3">
              <span className="h-px w-8 bg-[#C9A227]" />
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#A98216]">Selected Work</span>
              <span className="h-px w-8 bg-[#C9A227]" />
            </div>
          ...
          {filteredImages.length > 0 ? (
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {filteredImages.map((item, index) => (
                <article key={item.id} className="group overflow-hidden rounded-2xl border border-[#E6E2D7] bg-white shadow-sm transition-all duration-500 hover:-translate-y-1 hover:border-[#C9A227]/40 hover:shadow-xl">
                  <button type="button" onClick={() => openImage(item)} className="relative block w-full cursor-zoom-in text-left">
                    <div className="relative h-[240px] overflow-hidden">
                      <img src={item.image} alt={item.title} loading="lazy" onError={(e) => { e.target.onerror = null; e.target.src = '/images/placeholder.svg'; }} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                      ...
                      <div className="absolute bottom-4 left-4 right-4">
                        <span className="inline-block rounded-full border border-[#C9A227] bg-[#C9A227] px-3 py-1 text-[9px] font-bold uppercase tracking-wider text-[#11130F]">{item.category}</span>
                        <h3 className="mt-2 text-lg font-bold text-white">{item.title}</h3>
                      </div>
                    </div>
                  </button>
                  <div className="flex items-center justify-between px-5 py-4">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#9CA3AF]">SAAM Infrastructure</span>
      ...
      <section className="bg-[#11130F] px-5 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#1B2A4A] to-[#253D5E] p-8 text-center sm:p-12 lg:p-16">
            <div className="flex items-center justify-center gap-3">
              <span className="h-px w-8 bg-[#C9A227]" />
              <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#C9A227]">Start Your Project</span>
              <span className="h-px w-8 bg-[#C9A227]" />
            </div>
          ...
          <div className="mx-auto flex min-h-[100dvh] w-full max-w-7xl flex-col items-center px-3 pb-5 pt-[76px] sm:px-5 sm:pb-6">
            <div className="relative flex w-full flex-1 items-center justify-center px-9 sm:px-16 lg:px-20">
              ...
              <div
                ...
                onDoubleClick={() => setZoom((z) => (z === 1 ? 2.5 : (setPosition({ x: 0, y: 0 }), 1)))}
            ...
            <div className="mt-4 w-full max-w-5xl shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-[#202522]/95 shadow-2xl backdrop-blur-xl sm:rounded-3xl">
              <div className="p-5 text-center sm:p-6">
                ...
                <p className="mx-auto mt-3 max-w-4xl text-[15px] leading-7 text-white/65">{selectedImage.description}</p>
              </div>
            </div>
          </div>
        </div>
